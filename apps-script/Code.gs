...
function doPost(e) {
  try {
    ...
    // Owner-name enrichment: scrape the lead's website for the owner/founder, store it
    // on the lead (powers the {owner} email token). One lead, or a capped batch.
    if (a === 'enrichOwner') {
      ...
      if (!String(lead.website || '').trim()) return err_('No website on this lead to check.');
      const name = fetchOwnerName_(lead.website);
      if (name) patchLeadFields_(leadId, { ownerName: name, updatedAt: new Date().toISOString() });
      return ok({ ownerName: name || '', found: !!name });
    }
    if (a === 'enrichOwnersBatch') {
      const all = toObjs(getSheet(SHEETS.leads, LEAD_HDR));
      const todo = all.filter(function(l){ return String(l.website || '').trim() && !String(l.ownerName || '').trim(); }).slice(0, 15);
      let found = 0;
...
function cadenceRender(body, lead, company, agent) {
  ...
  const map = {
    ...
    // {owner} → the owner's FIRST name when enriched, else "there" so "Hi {owner}," still reads naturally.
    owner: (String(lead.ownerName || '').trim().split(/\s+/)[0] || 'there'),
  };
  return String(body || '')
...
/* ── Owner-name enrichment (website About/team scrape → {owner} personalization) ── */
...
function htmlToText_(html) {
  return String(html || '')
    ...
    .replace(/<[^>]+>/g, ' ')
    .replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&#?[a-z0-9]+;/gi, ' ')
    .replace(/\s+/g, ' ').trim();
}
// Fetch a business website (home + likely About/Team pages) and have Gemini name the
// owner/founder/principal. Returns '' if none is confidently found.
function fetchOwnerName_(website) {
  if (!website) return '';
  let base = String(website).trim();
  if (!/^https?:\/\//i.test(base)) base = 'https://' + base;
  base = base.replace(/\/+$/, '');
  const paths = ['', '/about', '/about-us', '/our-team', '/team', '/meet-the-team', '/staff', '/leadership'];
  let text = '';
  for (let i = 0; i < paths.length && text.length < 9000; i++) {
    try {
      const res = UrlFetchApp.fetch(base + paths[i], { muteHttpExceptions: true, followRedirects: true, validateHttpsCertificates: false, headers: { 'User-Agent': 'Mozilla/5.0 (compatible; AxiusBot/1.0)' } });
      if (res.getResponseCode() === 200) { const t = htmlToText_(res.getContentText()); if (t) text += ' ' + t; }
    } catch (e) {}
  }
  text = text.trim();
  if (text.length < 30) return '';
  return geminiOwner_(text.slice(0, 9000));
}
// Extract just the owner's full name from page text. Returns '' if not identifiable.
function geminiOwner_(text) {
  if (!GEMINI_API_KEY || GEMINI_API_KEY.indexOf('TU_') === 0) return '';
  const sys = 'You identify the OWNER, FOUNDER, PRINCIPAL, or PRESIDENT (an individual person) of a small business from its website text. ' +
    'Return ONLY that person\'s full name, exactly as written (e.g., "Jane Smith" or "Dr. Andrew Vance"). ' +
    'If no individual owner/founder/principal is clearly identifiable, or it is a large chain/franchise with no named owner, return exactly "NONE". ' +
    'Do not guess from generic staff or testimonials. Output the name only, nothing else.';
  try {
    const url = 'https://generativelanguage.googleapis.com/v1beta/models/' + GEMINI_MODEL + ':generateContent?key=' + encodeURIComponent(GEMINI_API_KEY);
    const payload = { systemInstruction: { parts: [{ text: sys }] }, contents: [{ role: 'user', parts: [{ text: 'Website text:\n' + text + '\n\nOwner full name:' }] }], generationConfig: { temperature: 0, maxOutputTokens: 20 } };
    const res = UrlFetchApp.fetch(url, { method: 'post', contentType: 'application/json', payload: JSON.stringify(payload), muteHttpExceptions: true });
    if (res.getResponseCode() !== 200) return '';
    const d = JSON.parse(res.getContentText() || '{}');
    let name = (d.candidates && d.candidates[0] && d.candidates[0].content && d.candidates[0].content.parts) ? d.candidates[0].content.parts.map(function(p){ return p.text || ''; }).join('').trim() : '';
    name = name.replace(/^["']|["']$/g, '').trim();
    // Sanity: 2–4 words, letters only (allow ., -, '), not "NONE".
...
function geminiPersonalizeEmail_(lead, cfg) {
  ...
  const system = 'You are ' + agent + ' from Axius, writing one short cold email to a local business owner, following a STRICT framework. ' +
    ...
    '1) "Hi {owner}," on its own line. 2) One short line that you came across ' + (lead.name || 'this business') + ' and wanted to reach out. ' +

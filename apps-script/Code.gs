// AXIUS LEADS & CRM — Apps Script (production)
// Fill in ALL constants before deploying.
// Hardened: push upserts by id (no silent lead loss), saveCommission is locked
// against double-payout races, and call recordings are private (consent/PII).
...
const TWILIO_FROM_NUMBER = '+15551234567'; // Fallback — per-agent numbers override this
const DRIVE_FOLDER_ID    = 'YOUR_DRIVE_FOLDER_ID';
// Claude API key — set via: File → Project settings → Script properties → CLAUDE_API_KEY
function getClaudeKey() { return PropertiesService.getScriptProperties().getProperty('CLAUDE_API_KEY') || ''; }
const SHEETS = { leads:'Leads', calls:'Calls', team:'Team', commissions:'Commissions', scripts:'Scripts' };
...
const LEAD_HDR = ['id','name','phone','address','website','rating','reviews','city','barrio','keyword','source','sourceDetail','status','providerId','closerId','dealValue','collectedAmount','closerCommission','commissionStatus','lockedBy','lockedUntil','assignedAt','workHistory','dncReason','followUpDate','notes','importedAt','updatedAt','calendarEventId','refundAmount','refundReason','refundedAt','pipelineType','personaType','contactCategory','relationshipPath','roleDescription','talentTier','rosterSize','icpScore','dealFlow','aiAwareness','decisionAccess','regulatoryExposure','strategicValue','bant','wwwhn','discoveryNotes','vectors','externalMeetings','nextStep','estimatedDealValue','expectedSalesCycle','dealSequence','originatorId','accountManagerId','originationDate','territory','keyContacts','accountNotes','lastStrategicTouchpoint'];
const CALL_HDR = ['id','leadId','leadName','phone','callSid','outcome','duration','notes','recordingUrl','driveUrl','consentConfirmed','calledAt'];
const TEAM_HDR = ['id','name','role','pinHash','providerRate','closerRate','contact','active','createdAt','territory','quarterSignCount','currentTier','tierHistory','twilioNumber'];
const COMM_HDR   = ['id','leadId','leadName','dealValue','dealSequence','platformCommissionRate','platformCommission','closerId','closerName','anchorTierRate','closerAmount','anchorResidual','leadgenId','leadgenName','leadgenTierRate','leadgenResidual','warmLeadSplit','quarter','year','status','createdAt','paidAt','paidBy','paymentRef','refundReason','adjustedBy','adjustedAt','overrideNote','collectedAmount'];
const SCRIPT_HDR = ['id','name','stage','body','createdAt','updatedAt'];
const KNOWLEDGE_HDR = ['id','documentName','documentType','stage','category','title','body','personaTarget','tags','createdAt','updatedAt'];
...
function toObjs(sheet) {
  ...
  return rows.slice(1).map(r => {
    ...
    if(o.bant) try{o.bant=JSON.parse(o.bant)}catch(e){o.bant={};}
    if(o.wwwhn) try{o.wwwhn=JSON.parse(o.wwwhn)}catch(e){o.wwwhn={};}
    if(o.discoveryNotes) try{o.discoveryNotes=JSON.parse(o.discoveryNotes)}catch(e){o.discoveryNotes={};}
    if(o.vectors) try{o.vectors=JSON.parse(o.vectors)}catch(e){o.vectors=[];}
    if(o.externalMeetings) try{o.externalMeetings=JSON.parse(o.externalMeetings)}catch(e){o.externalMeetings=[];}
    if(o.keyContacts) try{o.keyContacts=JSON.parse(o.keyContacts)}catch(e){o.keyContacts=[];}
    if(o.accountNotes) try{o.accountNotes=JSON.parse(o.accountNotes)}catch(e){o.accountNotes=[];}
    if(o.tierHistory) try{o.tierHistory=JSON.parse(o.tierHistory)}catch(e){o.tierHistory=[];}
    return o;
  });
}
...
function doGet(e) {
  try {
    ...
    if (a === 'pull') {
      ...
      const knowledge = toObjs(getSheet('Knowledge', KNOWLEDGE_HDR));
      return ok({leads,calls,team,commissions:comms,scripts,knowledge,serverTime:new Date().toISOString()});
    }
    ...
    if (a === 'twiml') {
      // Look up agent's assigned Twilio number, fallback to global default
      let fromNum = TWILIO_FROM_NUMBER;
      const agentId = e.parameter.agentId || '';
      if (agentId) {
        const team = toObjs(getSheet(SHEETS.team, TEAM_HDR));
        const agent = team.find(m => m.id === agentId || m.name === agentId);
        if (agent && agent.twilioNumber) fromNum = agent.twilioNumber;
      }
      const xml = ContentService.createTextOutput(
        '<?xml version="1.0"?><Response><Dial callerId="'+fromNum+'" record="record-from-answer" recordingStatusCallback="'+ScriptApp.getService().getUrl()+'?action=rec_hook"><Number>'+(e.parameter.To||'')+'</Number></Dial></Response>'
...
function doPost(e) {
  try {
    ...
    if (b._secret !== CRM_SECRET) {
      return err_('Unauthorized — configure CRM_SECRET in Code.gs with the value from Settings.');
    }
    if (a === 'push') {
      ...
      const jsonFields = new Set(['notes','workHistory','bant','wwwhn','discoveryNotes','vectors','externalMeetings','keyContacts','accountNotes']);
      let added=0;
      (b.data||[]).forEach(l=>{
        const id=String(l.id||'').trim();
        if(id && !ex.has(id)){
          s.appendRow(LEAD_HDR.map(h=>jsonFields.has(h)?JSON.stringify(l[h]||[]):(l[h]??'')));
          ex.add(id); added++;
        }
      });
      return ok({added});
        ...
        if(String(rows[i][ic])===String(b.id)){
          ...
          h.forEach((col,ci)=>{
            if(Object.prototype.hasOwnProperty.call(b,col)){
              const jf = new Set(['notes','workHistory','bant','wwwhn','discoveryNotes','vectors','externalMeetings','keyContacts','accountNotes']);
              s.getRange(i+1,ci+1).setValue(jf.has(col)?JSON.stringify(b[col]||[]):(b[col]??''));
            }
          });
    ...
    if (a === 'sendSMS') {
      const {to, body:msgBody, agentId:smsAgentId} = b;
      if (!to || !msgBody) return err_('to and body required');
      // Look up agent's Twilio number, fallback to global
      let smsFrom = TWILIO_FROM_NUMBER;
      if (smsAgentId) {
        const team = toObjs(getSheet(SHEETS.team, TEAM_HDR));
        const ag = team.find(m => m.id === smsAgentId);
        if (ag && ag.twilioNumber) smsFrom = ag.twilioNumber;
      }
      const auth = Utilities.base64Encode(TWILIO_ACCOUNT_SID + ':' + TWILIO_AUTH_TOKEN);
      const res  = UrlFetchApp.fetch(
        'https://api.twilio.com/2010-04-01/Accounts/' + TWILIO_ACCOUNT_SID + '/Messages.json',
        {method:'post', headers:{Authorization:'Basic '+auth},
         payload:{From:smsFrom, To:to, Body:msgBody},
    ...
    if (a === 'inbound') {
      ...
      const lead={
        ...
        status:b.status||'Target',dncReason:'',followUpDate:'',
        ...
        pipelineType:'supply',
        bant:JSON.stringify({}),wwwhn:JSON.stringify({}),discoveryNotes:JSON.stringify({}),
        vectors:JSON.stringify([]),externalMeetings:JSON.stringify([]),
        keyContacts:JSON.stringify([]),accountNotes:JSON.stringify([]),
        lastStrategicTouchpoint:'',
        importedAt:now,updatedAt:now,
      };
      s.appendRow(LEAD_HDR.map(h=>lead[h]??''));
      return ok({added:true,duplicate:false});
    }
    if (a === 'processDocument') {
      if (!getClaudeKey()) return err_('Claude API key not configured in Code.gs');
      const {documentName, documentText, documentType} = b;
      if (!documentText) return err_('documentText required');

      const prompt = `You are an AI sales knowledge extractor for AIV, a company that certifies, protects, and licenses AI-generated digital identities.

Analyze this sales document and extract structured knowledge cards. For EACH card, provide:
- stage: one of "opening", "pitch", "objections", "close", "rebuttals", or "general"
- category: one of "script", "objection_response", "talking_point", "competitive", "persona_specific", "process"
- title: short descriptive title (max 60 chars)
- body: the actual content (scripts, responses, talking points)
- personaTarget: one of "all", "manager", "talent", "agent", "lawyer", "brand", "developer" or comma-separated
- tags: comma-separated keywords for search

Return ONLY a JSON array of objects with these fields. No markdown, no explanation, just the JSON array.
...
${documentText.slice(0, 50000)}`;
      ...
      try {
        ...
        const d = JSON.parse(res.getContentText());
        if (d.error) return err_(d.error.message || 'Claude API error');

        const text = d.content && d.content[0] && d.content[0].text || '';
        let cards;
        try {
          // Try to parse the response as JSON - handle cases where Claude wraps in markdown
          const jsonMatch = text.match(/\[[\s\S]*\]/);
          cards = jsonMatch ? JSON.parse(jsonMatch[0]) : JSON.parse(text);
        } catch(pe) {
          return err_('Failed to parse Claude response as JSON: ' + pe.message);
        }

        // Save each card to Knowledge sheet
        const ks = getSheet('Knowledge', KNOWLEDGE_HDR);
        const now = new Date().toISOString();
        const saved = [];
        (cards || []).forEach(card => {
          const rec = {
            ...
            stage: card.stage || 'general',
            category: card.category || 'general',
            title: card.title || '',
            body: card.body || '',
            personaTarget: card.personaTarget || 'all',
            tags: card.tags || '',
            createdAt: now,
            updatedAt: now,
          };
          ks.appendRow(KNOWLEDGE_HDR.map(h => rec[h] ?? ''));
          saved.push(rec);
        });

        return ok({saved: saved.length, cards: saved});
      } catch(e) {
        return err_('Claude API call failed: ' + e.message);
      }
    }
    if (a === 'pullKnowledge') {
      const ks = getSheet('Knowledge', KNOWLEDGE_HDR);
      const knowledge = toObjs(ks);
      return ok({knowledge});
    }
    if (a === 'deleteKnowledge') {
      const ks = getSheet('Knowledge', KNOWLEDGE_HDR);
      const rows = ks.getDataRange().getValues();
      const h = rows[0], ic = h.indexOf('id');
      for (let i = rows.length - 1; i >= 1; i--) {
        if (String(rows[i][ic]) === String(b.id)) { ks.deleteRow(i + 1); break; }
      }
      return ok({deleted: true});
    }
...
function sendWeeklyReport() {
  ...
  const interAll   = leads.filter(l => l.status === 'Engaged').length;
  const dncAll     = leads.filter(l => l.status === 'Do Not Contact').length;
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a1a2e">
  ...
  <div style="background:#f9f9fb;padding:24px 32px;border-radius:0 0 12px 12px">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      ...
      <tr style="background:#f0f0f5"><td style="padding:8px 12px;border-radius:6px">Engaged</td><td style="padding:8px 12px;text-align:right;font-weight:600;color:#2dd4bf">${interAll}</td></tr>
      <tr><td style="padding:8px 12px">Do Not Contact</td><td style="padding:8px 12px;text-align:right;color:#888">${dncAll}</td></tr>
    </table>
  </div>
...
function runScheduledScrapes() {
  if (!PLACES_API_KEY || PLACES_API_KEY === 'YOUR_GOOGLE_PLACES_API_KEY') { Logger.log('PLACES_API_KEY not configured'); return; }
  const ss   = SpreadsheetApp.openById(SHEET_ID);
  const cfg  = ss.getSheetByName('Config');
  if (!cfg) return;
    ...
    leads.forEach(l => {
      ...
      if (phone && phone !== 'N/A' && !existing.has(phone)) {
        const lead = {
          ...
          status:'Target', dncReason:'', followUpDate:'', notes:JSON.stringify([]),

...
- [ ] **Cadence engine** (BUILT — CRM-native, deterministic; no Vercel/LLM needed). Ships in **dry-run**: use Admin → Secuencias de outreach → "Ejecutar ahora" to preview who it would enroll/message — it writes/sends nothing. **Tune it from the UI** (no code edit): the Secuencias panel now has the daily cap, signing person (`{agente}`), company (`{empresa}`), quiet hours, days-between-touches, CAN-SPAM postal address, and an **"Enviar mensajes reales (en vivo)"** toggle (with a confirmation). To go live **after** SMS/WhatsApp provisioning: fill those fields, check the live toggle + Guardar, then "Activar" the hourly trigger. (The `Code.gs` constants remain the defaults/fallback.) It auto-enrolls every eligible lead and sends the templated multi-step cadence, honoring opt-out, quiet hours and the daily cap.

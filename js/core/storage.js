...
function saveLocal() {
  try {
    ...
    // Persist the outbound mutation queue so a failed write (commission, engagement,
    // team, etc.) survives a reload and is retried, instead of being silently lost.
    localStorage.setItem('aiv-mutq', JSON.stringify(S.mutationQueue || []));
...
function loadLocal() {
  ...
  try { S.mutationQueue  = JSON.parse(localStorage.getItem('aiv-mutq') || '[]'); } catch(e) { S.mutationQueue = []; }
